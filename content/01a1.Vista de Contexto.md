...
|Autor||
|Fuente||
|Versión|{% if manubot.ci_source is defined -%} [@{{manubot.ci_source.commit | truncate(length=7, end='', leeway=0)}}](https://github.com/{{manubot.ci_source.repo_slug}}/tree/{{manubot.ci_source.commit}}) {% endif -%} del {{manubot.date}}|
